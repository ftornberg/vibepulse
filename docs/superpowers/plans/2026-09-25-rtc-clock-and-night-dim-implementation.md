...
- [ ] Handover: after merge the owner OTA-pushes (panel on its charger) and runs physical steps 3 and 4 from the spec: boot with WiFi disabled shows the right time from the RTC (log `tid från RTC`), and night dimming with a temporarily moved window (edit `TG_NIGHT_START_HHMM` in `secrets.h`, rebuild, OTA, watch `natt: dimmar` and the glass). The first boot after this firmware will still log `RTC opålitlig` (a fresh chip says year 2000) until the first SNTP write-back; the second boot is the real test.
