...
static void test_text(void) {
  ...
  check("invalid clock shows dashes", strcmp(buf, "--:--") == 0);
  tg_time_clock_text(true, 24, 0, buf, sizeof buf);
  check("hour 24 shows dashes", strcmp(buf, "--:--") == 0);
  tg_time_clock_text(true, 12, 60, buf, sizeof buf);
  check("minute 60 shows dashes", strcmp(buf, "--:--") == 0);
  tg_time_clock_text(true, -1, 0, buf, sizeof buf);
  check("negative hour shows dashes", strcmp(buf, "--:--") == 0);
...
/* "HH:MM", eller "--:--" när klockan är ogiltig eller siffrorna utanför
...
static void test_clock(void) {
  ...
  check("invalid clock shows dashes", strcmp(m.big, "--:--") == 0);
  tg_time_present(&m, TG_TIME_MODE_CLOCK, true, 24, 0, &p, &c, 0);
  check("hour 24 shows dashes", strcmp(m.big, "--:--") == 0);
}
...
static void test_timer_mode(void) {
  ...
  check("idle timer shows dashes, not zero", strcmp(m.big, "--:--") == 0);
...
Expected: builds `torget-sim` with no errors or new warnings.
...
Run:
```bash
cmake -S sim -B sim/build -G Ninja -DTORGET_WITH_TIME=ON && ninja -C sim/build
...
```bash
cmake -S sim -B sim/build -G Ninja -DTORGET_WITH_TIME=ON && ninja -C sim/build
TORGET_CAPTURE_DIR=/tmp/time-frames sh -c 'mkdir -p /tmp/time-frames && ./sim/build/torget-sim --time-app-captures'
ls /tmp/time-frames
```
Expected: 14 files `torget-time-*.bmp` (clock, clock-unset, pomodoro idle/running/paused/done/break, done-over-clock, timer select/running/done). Confirm each is 480 x 480: `python3 -c "from PIL import Image; import glob; [print(f, Image.open(f).size) for f in sorted(glob.glob('/tmp/time-frames/*.bmp'))]"`.

- [ ] **Step 3: Owner review gate (static frames first)**
...
- [ ] **Step 4: Commit the capture mode (and any approved layout changes)**

```bash
git add sim/main.c components/app_time
...
```
Expected: the build finishes and prints the app binary size; the log contains no error from `app_time` and no `undefined reference to torget_orientation`. Then confirm the default build still excludes the app: `idf.py -B build-time -DTORGET_WITH_TIME=OFF reconfigure` prints no TID line and `grep -c "app_time" build-time/build.ninja` is `0` or the component is not linked into the app.

**Do not flash, do not run `tools/ota-flash.sh`.** A flash needs the owner's explicit request, and this branch's binary is `-dirty` until merged.
