...
- **No flash and no OTA** unless the owner explicitly asks. Build firmware only into a directory OUTSIDE the repository (`/tmp/torget-time-build`), never into any repo-root `build*/`: the tokenserver announces the newest `build*/torget.bin` to the panel as UPDATE READY, so a branch or `-dirty` build there pops an update screen on the desk panel (seen 2026-09-25).
...
## Amendment A: the ring (owner request, 2026-09-25, after this plan was written)

The spec now has a "Ring" section. Where it disagrees with the task code below, this amendment wins:

1. **Task 1 (`time_core`)** gains two pure helpers, declared in `time_core.h` and tested in `test_time_core.c`:
   - `int tg_ring_seconds(bool valid, int second);` returns `-1` when `!valid` or `second` is outside 0..59, else `(second + 1) * 1000 / 60` (0 s gives 16, 29 s gives 500, 59 s gives 1000).
   - `int tg_ring_remaining(const tg_timer *t, int64_t now_us);` returns `-1` unless the timer is RUNNING or PAUSED with `total_us > 0`; otherwise the remaining fraction in permille, rounded up (`(remaining * 1000 + total - 1) / total`, clamped 0..1000), so it is never 0 while running.
2. **Task 2 (`time_present`)**: `tg_time_present` takes an extra `int second` after `minute`; `tg_time_view_model` gains `int ring_permille` (`-1` hides the ring, initialised like `dots_done`). Clock mode uses `tg_ring_seconds`, pomodoro and timer modes use `tg_ring_remaining` on their own timer, and the DONE marker sets `-1`. Every existing test call gets `0` for `second`.
3. **Task 4 (`time_views`)**: an `lv_arc` ring (448 x 448 at 16,16; rotation 270; range 0..1000; knob removed; not clickable; 10 px arc width; `COL_TRACK` background arc, `COL_ACCENT` indicator; transparent background, no border, no padding) is created before all labels. Layout moves inside the ring: the big number is `lv_obj_align`ed to the centre (offset -8, or -70 while the presets show), the caption sits at y 112, the hint at y 300 (y 236 while the presets show), the preset chips are 96 x 64 showing only the number, in a 324 px row at y 296, and RESET sits at y 396. `app_time.c` passes `local.tm_sec` as `second`.

Layout numbers are a first draft; the Task 5 review gate settles them.
...
```bash
grep -c "time_app" /tmp/torget-time-build/torget.map      # ON: expect > 0 (the registry references &time_app)
idf.py -B /tmp/torget-time-build -DTORGET_WITH_TIME=OFF build
grep -c "time_app" /tmp/torget-time-build/torget.map      # OFF: expect 0
```
If `torget.map` has another name, `ls /tmp/torget-time-build/*.map`.
