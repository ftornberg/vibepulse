...
- **Percent** is the PMU fuel gauge rounded to an integer. No estimation from
  ...
  number. Invalid means: a byte above 100, or 0 % while the cell measures
  3.5 V or more (a lithium cell at that voltage is not empty; a gauge that
  says so is unconfigured or disabled, and must not walk the panel into
  LOW, CRITICAL and a shutdown). The rule is pure, `tg_batt_gauge_percent`.
